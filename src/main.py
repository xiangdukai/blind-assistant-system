...
class BlindAssistantSystem:
    ...
    def main_loop(self):
        ...
        while self.running:
            ...
            # 步骤1: 目标检测 (YOLO)
            danger_detections, scene_detections = self._yolo_detect(color_image)

            # 步骤2: 多目标跟踪（3D空间跟踪，使用卡尔曼滤波）
            tracked_detections = self.tracker.update(danger_detections, depth_map)
            ...
            # 步骤4: 场景检测 (按优先级调度)
            scene_modules = {
                'stair_detection': [self.stair_detector.detect, self._handle_stair],  # 楼梯检测
                'blind_path_detection': [self.blind_path_detector.detect, self._handle_blind_path],  # 盲道检测
                'crosswalk_detection': [self.crosswalk_detector.detect, self._handle_crosswalk],  # 斑马线
                'ocr': [self.ocr_recognizer.recognize, self._handle_ocr]  # 店铺
                # 可以根据需要继续添加其他模块
            }

            # 将整张图像和所有检测信息传给各个场景模型
            for scene_name, (model_fn, handler_fn) in scene_modules.items():
                if self.config[scene_name]['enabled']:
                    print(f"触发 {scene_name} 模型处理...")
                    # 让每个模型自己处理自己关心的检测
                    result = model_fn(color_image, scene_detections, depth_map)  
                    handler_fn(result)
    ...
    def _yolo_detect(self, image):
        ...
        danger_class_ids = self.config.get('danger_class_ids', [])  # 配置危险目标类别
        scene_class_ids = self.config.get('scene_class_ids', [])    # 配置场景目标类别
        ...
        for r in results:
            for box in r.boxes:
                ...
                if class_id in danger_class_ids:
                    danger_detections.append(detection)  # 添加到危险目标列表
                elif class_id in scene_class_ids:
                    scene_detections.append(detection)   # 添加到场景目标列表

        return danger_detections, scene_detections
